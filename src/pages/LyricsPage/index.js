...
const LyricsPage = () => {
    ...
    const renderLyrics = () => {
        return songs.map(s => 
            <div key={s.id} className="lyrics-container">
                <div className="lyrics-row">
                    <h2>
                        <b>{s.title}</b>
                    </h2>
                    { showLyrics ? <Lyrics title={s.lyrics} close={toggleLyrics} /> : <button className="lyrics-btn" onClick={toggleLyrics}>Click for lyrics</button>}
            </div>
            </div>
        );
    };
